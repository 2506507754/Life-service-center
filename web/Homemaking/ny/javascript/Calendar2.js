document.write("<div id=meizzCalendarLayer style='position: absolute; z-index: 9999; width: 148; height: 193; display: none'>");
document.write("<iframe name=meizzCalendarIframe scrolling=no frameborder=0 width=100% height=100%></iframe></div>");

function writeIframe() {
  var strIframe = "<html><head><meta http-equiv='Content-Type' content='text/html; charset=gb2312'><style>" +
    "*{font-size: 12px; font-family: 宋体}" + ".bg{  color: " +
    WebCalendar.lightColor + "; cursor: default; background-color: " +
    WebCalendar.darkColor + ";}" + "table#tableMain{ width: 146; height: 180;}" +
    ...
    "       color:" + WebCalendar.wordColor + "; background-color: " +
  ...
  if (WebCalendar.drag) {
    ...
    "function document.onkeydown(){ if (window.event.srcElement.id=='meizzHour'||window.event.srcElement.id=='meizzMin') return;  switch(window.event.keyCode){  case 27 : parent.hiddenCalendar(); break;" +
  ...
  strIframe += "<select name=tmpYearSelect onblur='parent.hiddenSelect(this)' style='z-index:1;position:absolute;top:3;left:18;display:none'" +
    ...
    "</td></tr><tr><td height=20><table id=tableWeek border=1 width=100% cellpadding=0 cellspacing=0 ";

  if (WebCalendar.drag) {
    strIframe += "onmousedown='setCapture(); dragStart()' onmouseup='releaseCapture();drag=false' onmouseout='drag=false'";
  }
  strIframe += " borderColorLight='" + WebCalendar.darkColor +
    "' borderColorDark='" + WebCalendar.lightColor + "'>" +
    ...
    "    <table id=tableDay height=120 width=100% border=0 cellspacing=0 cellpadding=0>";
  ...
  //strIframe += "<td colspan=3><input name=prevHour title='前一小时' onclick='meizzHour.value=(parseInt(meizzHour.value)+23)%24;' type=button value='&lt;' onfocus='this.blur()'>";
  strIframe += "<td class=out colspan=3 align='center'><input type=input  id=meizzHour size=1 maxlength=2>";
  strIframe += ":";
  strIframe += "<input type=input  id=meizzMin size=1 maxlength=2 ></td>";
  //strIframe += "<input name=prevHour title='后一小时' onclick='meizzHour.value=(parseInt(meizzHour.value)+1)%24;' type=button value='&gt;' onfocus='this.blur()'></td>";
  strIframe += "<td colspan=2 class=out title='" + WebCalendar.regInfo +
    "'><input style=' background-color: " + WebCalendar.btnBgColor +
    ";cursor: hand; padding-top: 4px;  height: 100%; border: 0' " +
    " type=button value='√' id='meizzOk' onclick='parent.returnDate(\"meizzOk\")' " +
    "onmouseover=' this.style.color=parent.WebCalendar.todayColor;' " +
    "onmouseout='this.style.color=parent.WebCalendar.wordColor' " +
    "><input style=' background-color: " + WebCalendar.btnBgColor +
    ";cursor: hand; padding-top: 4px;  height: 100%; border: 0' onfocus='this.blur()'" +
    " type=button value='×' onclick='parent.hiddenCalendar()'" +
    "onmouseover=' this.style.color=parent.WebCalendar.todayColor;' " +
    "onmouseout='this.style.color=parent.WebCalendar.wordColor' " +
    "></td></tr></table>" + "</td></tr><tr><td height=20 width=100% bgcolor='" +
    WebCalendar.lightColor + "'>" + "    <table border=0 cellpadding=1 cellspacing=0 width=100%>" +
...
function WebCalendar() {
  ...
  this.thisMin    = new Date().getMinutes();
...
function calendar() {
    ...
    if (WebCalendar.objExport.value.trim() != "") {
      ...
      if (WebCalendar.dateStyle == null) {
        ...
        if (WebCalendar.timeShow) {
          ...
          WebCalendar.thisMin = parseInt(WebCalendar.dateStyle[6], 10);
        }
  ...
  WebCalendar.inputDate = parseInt(WebCalendar.thisDay, 10) + "/" +
    parseInt(WebCalendar.thisMonth, 10) + "/" + parseInt(WebCalendar.thisYear, 10);
}
...
function writeCalendar() {
  ...
  if (WebCalendar.thisHour.toString() != '') {
    WebCalendar.iframe.document.all.meizzHour.value = appendZero(WebCalendar.thisHour).toString();
  }
  if (WebCalendar.thisMin.toString() != '') {
    WebCalendar.iframe.document.all.meizzMin.value = appendZero(WebCalendar.thisMin).toString();
  }
  ...
  if (!(y <= 9999 && y >= 1000 && parseInt(m, 10) > 0 && parseInt(m, 10) < 13 && parseInt(d, 10) > 0)) {
    ...
    for(var i=0; i<37; i++)    //这个循环是根据源数组写到日历里显示
    {
       ...
       //alert(WebCalendar.iframe.document.all.meizzOk.onclick)
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        var returnValue;
        var inputDate;
        //alert(WebCalendar.inputDate)
        if (arguments.length==0)
        inputDate= WebCalendar.day[this.id.substr(8)];
        else
        inputDate=(arguments[0]=='meizzOk') ? WebCalendar.inputDate:arguments[0];

        if (inputDate=='') {hiddenCalendar();return;}
        ...
        if(WebCalendar.timeShow)
        {
            ...
            var h = WebCalendar.iframe.document.all.meizzHour.value;
            var m = WebCalendar.iframe.document.all.meizzMin.value;
            var s = 0;
            re = /^\d{1,2}$/;
            if (!re.test(h) || h > 23 || h < 0) {
              alert("你设定的日期输出格式不对！\r\n\r\n请重新定义 WebCalendar.format ！");
              WebCalendar.iframe.document.all.meizzHour.value = "";
              WebCalendar.iframe.document.all.meizzHour.focus();
              return false;
            }
            if (!re.test(m) || m > 59 || m < 0) {
              alert("你设定的日期输出格式不对！\r\n\r\n请重新定义 WebCalendar.format ！");
              WebCalendar.iframe.document.all.meizzMin.value = "";
              WebCalendar.iframe.document.all.meizzMin.focus();
              return false;
            }
